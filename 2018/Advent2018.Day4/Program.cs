...
namespace Advent2018.Day4
{
    ...
    {
        #region
        private static readonly string input = @"[1518-03-25 00:01] Guard #743 begins shift
...
[1518-03-02 00:02] Guard #743 begins shift";
        ...
        private static Dictionary<int, int[]> GetGuardMinutes()
        {
            ...
            {
                if (i.Contains("begins shift"))
                {
                    currentGuardId = GetGuardId(i);
                }
                else if (i.Contains("falls asleep"))
                {
                    startDate = DateTime.Parse(i.Substring(1, 16));
                }
                else if (i.Contains("wakes up"))
                {
                    endDate = DateTime.Parse(i.Substring(1, 16));
                    int[] minuteCounts;

                    if (!minutes.TryGetValue(currentGuardId, out minuteCounts))
                    {
                        minuteCounts = new int[60];
                        minutes.Add(currentGuardId, minuteCounts);
                    }

                    while (startDate < endDate)
                    {
                        minuteCounts[startDate.Minute]++;
        ...
        {
            var regex = new Regex(@".{18} Guard #(\d+) begins shift");
